...
async function fetchData() {
  try {
    ...
    document.querySelector("#destination-description p").innerHTML =
      data.deskripsi;
    document.querySelector(".bg-[#EADBD1]").textContent = data.kategori;
    document.querySelector(
      ".text-gray-600"
    ).textContent = `Mulai dari: ${data.harga_tiket}`;
...
function generateStars(rating) {
  let stars = "";
  for (let i = 0; i < 5; i++) {
    stars += `<img src="/frontend/src/asset/icon/star-yellow.png" alt="star" class="w-4 ${
      i < rating ? "opacity-100" : "opacity-60"
    }">`;
  }
  return stars;
}
