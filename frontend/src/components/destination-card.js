class destinationCard extends HTMLElement {
    ...
    // ⬇️ Tambahkan setter ini supaya bisa menerima `card.data = place`
    set data(place) {
      this.setAttribute("img", place.img);
      this.setAttribute("place_name", place.place_name);
      this.setAttribute("city", place.city);
      this.setAttribute("category", place.category);
      this.setAttribute("place_description", place.place_description);
    }
    ...
    render() {
        ...
        console.log(category); // Debugging untuk memeriksa apakah kategori sudah terisi
